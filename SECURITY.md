...
1. **GitHub private security advisory** (preferred). Go to the
   [Security tab](https://github.com/myICOR/icor-for-life-focus/security/advisories/new)
...
2. **Email** `support@myicor.com` with `SECURITY` and `icor-for-life-focus` in the subject
...
- **Local scope only.** It reads and renders vault content and stores its own
  settings through Obsidian's plugin settings API.
...
- Code execution triggered by note content: a crafted note, frontmatter value or
  embedded fragment that results in script execution, arbitrary file write, or
  process spawning when the plugin renders it.
- HTML or script injection into a rendered pane from untrusted note content.
- Path traversal: a note-controlled string that causes a read or write outside the
  vault, or outside the folders the plugin is configured to use.
- Any read or write of files the plugin has no reason to touch.
